...
import { Container, Row, Col, Button, Navbar as Navigation, Nav } from "react-bootstrap";
       ...
       { user && (
        <React.Fragment>
         <div className="user__name"> Hello {user.name}</div>
          <img className="user__picture" src={`${user.avatarURL}`} alt={`${user.name}'s avatar`} width={80} height={80}/>
